					...
					<tbody>
						...
						foreach ($result as $row) {  ?>
							<tr>
								...
								<?php if(1){ ?>
									<a href = "#" data-toggle="modal" data-target="#emailModal" data-senderName = "<?php echo $row['sender_name'];?>" data-emph = "<?php echo $row['email_phone'];?>"  data-name = "<?php echo $row['name']; ?>" data-slink = "<?php echo $site_url; ?><?php echo $row['unique_link'];?>" data-lang = "<?php echo $row['vs_lang_code']; ?>" class = "emailPopup">Send</a>
								<?php }else{ ?>
										<?php	
									
											$row['name'] = empty($row['name']) ? '' : $row['name'];
											$message =	str_replace("{name}", $row['name'],EMAIL_MESSAGE); 
											$message =	str_replace('{unique_link}', $site_url . "{$row['unique_link']}". "/{$row['vs_lang_code']}",$message);
											//echo $message;
											//%0A - line feed
										?>
									<a href = "#" data-toggle="modal" data-target="#emailModal" data-senderName = "<?php echo $row['sender_name'];?>" data-emph = "<?php echo $row['email_phone'];?>"  data-name = "<?php echo $row['name']; ?>" data-slink = "<?php echo $site_url; ?><?php echo $row['unique_link'];?>" data-lang = "<?php echo $row['vs_lang_code']; ?>" class = "emailPopup">Send via Server</a> | 
								...
								<?php 
									if(sizeof($row['answers'])>0){
										
										foreach($row['answers'] as $key=>$answers){
											echo "<p>".$answers['datetime']."</p>";
											echo "<div style='padding-left:10px;'>";
											echo "<ul style='margin:2px 0 0 20px; padding-left:0;'>";
											foreach($answers['answer_and_Comments'] as $answer){
												echo "<p style='margin-left:-20px;'>".$answer['vs_form_code']."</p>";
												echo "<li>";
												echo "<p><b>".$answer['answer']."</b></p>";
												if(!empty($answer['comment'])) echo "<p>Comment: ".$answer['comment']."</p>";
												echo "</li>";
											}
											echo "</ul>";
											echo "</div>";
								...
								<td><a href = "<?php echo $site_url; ?>?a=s&sid=<?php echo $row['unique_link'];?>&lang=<?php echo $row['vs_lang_code'];?>&preview=1" target = "_blank"><?php echo $row['unique_link'];?></td>
							</tr>
						<?php $sno++; }  ?>
					</tbody>
				</table>
	...
	$(document).on("click", ".emailPopup", function () {
		 ...
		 var emailPhone = $(this).data('emph');
		  ...
		  var lang = $(this).data('lang');
		   var unLink = $(this).data('slink') + "/" + lang;

        ...
        <?php if($_SESSION['users']['user_type'] == 'admin') { ?>
			...
			<form role="form" id = "link" method = "POST" >
				<div class="form-group">
					...
					<select name = "vs_survey" id = "survey_id" class="form-control" data-validation="required">
						<?php while ($row = mysql_fetch_assoc($surveyList)) {  ?>
							<option value = "<?php echo $row['vs_survey_id']."_".$row['vs_form_id']; ?>"><?php echo $row['name']; ?></option>
						<?php } ?>
					</select>
	...
	$("#sendVideoFrm" ).submit(function( event ) {
		...
		$.ajax({
			...
			$('#send-video').html('Send');
			 ...
			 message =	message.replace(/\{unique_link\}/g, sLink + response.userData.unique_link + lang); 

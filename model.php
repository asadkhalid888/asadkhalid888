...
class Model{
	...
	public function generateLink($data){
		...
		$vs_survey = explode("_", $vs_survey);
		$vs_survey_id = $vs_survey[0]; // vs_survey_id from survey
		$vs_form_id = $vs_survey[1];//vs_form_id from survey
		$sql = "INSERT into links (name,video_id,vs_users_id,vs_survey_id,vs_form_id) values('$name','$video_id',$user_id,$vs_survey_id,$vs_form_id);";
		mysql_query($sql);
	}
	...
	function listLinks(){
		...
		$sql = "SELECT * from links l where l.vs_team_id = $team_id and private  = 0 order by id desc limit 800";
		else
		$sql = "SELECT * from links l where private = 0 order by id desc limit 800";
	...
	function getVideoReport($video_id){
		...
		$user_id  = $_SESSION['users']['vs_users_id'];
		$sql = "SELECT sl.id as  survey_link_id ,sl.link_id, sl.unique_link,sl.name,sl.email_phone,sl.vs_lang_code, sl.vs_prospect_id from survey_links sl where sl.link_id = $video_id AND vs_users_id = $user_id order by sl.id asc ";
		//echo $sql;exit;
		$res = mysql_query($sql);
		$resultantData = array();
		while($submitQ = mysql_fetch_assoc($res)){
			$sql = "SELECT count(lh.viewed) as viewed,count(lh.video_started) as video_started, lh.ip_address,lh.added_on FROM `link_history` lh where lh.survey_link_id = {$submitQ['survey_link_id']} group by lh.survey_link_id order by lh.survey_link_id asc";
			$result = mysql_query($sql);
			$row = mysql_fetch_assoc($result);
			$sql = "Select anst.vs_form_id, anst.vs_prospect_id,anst.notes,anst.ans, anst.stampx, anst.link_id, anst.vs_survey_links_id,anst.session_id,f.vs_form_code as vs_form_code from vs_prospect_ans anst inner join vs_form f on anst.vs_form_id  = f.vs_form_id where vs_survey_links_id = '".$submitQ['survey_link_id']."' GROUP BY anst.session_id, anst.vs_form_id ORDER BY stampx";
			$res1 = mysql_query($sql);
			$row["answers"] = [];
			while($submitQ1 = mysql_fetch_assoc($res1)){
				

				$answers 	= unserialize($submitQ1['ans']);
				$answersArr = [];
				foreach($answers['vs_form_sets'] as $answer){
					$answersArr = $answer;
					break;
				}
				
				$sqlAns = "SELECT IF(tr.label IS NOT NULL,tr.label,fu_a.label) AS label,fu_a.label AS label_default,tr.label AS label_trans FROM vs_fu_actions fu_a LEFT JOIN vs_lang_trans tr ON (tr.vs_fu_actions_id = fu_a.vs_fu_actions_id AND tr.vs_lang_code = '') WHERE fu_a.vs_fu_actions_id = ".$answersArr['answer'];
				$resAns = mysql_query($sqlAns);
				$answerRec = mysql_fetch_assoc($resAns);
				if(!isset($row["answers"][$submitQ1['session_id']])){
					$row["answers"][$submitQ1['session_id']]['datetime'] = $submitQ1['stampx'];
				}
				$row["answers"][$submitQ1['session_id']]['answer_and_Comments'][]   = array(
					'answer'=>$answerRec['label_default'],
					'comment'=>$submitQ1['notes'],
					'vs_form_code'=>$submitQ1['vs_form_code']
				);				
			}
			...
			$row['vs_lang_code']= $submitQ['vs_lang_code'];
			$resultantData[] = $row;
		...
		//echo "<pre>"; print_r($resultantData);exit;
...
/*	function getVideoReport($video_id){
	...
	function addUser($data){
		...
		if(!$id){
			$sql = "INSERT into survey_links (link_id,name,email_phone,vs_users_id, vs_lang_code) values($video_id,'$name','$email_phone','$user_id','$lang');";
			mysql_query($sql);
			$last_insert_id = mysql_insert_id(); 
			$prefix = 'survey-' . $last_insert_id.'-';
			$uniqid = uniqid($prefix);
			...
			$name  = empty($name) ? '' : $name ; 
			$sql = "UPDATE survey_links SET email_phone = '$email_phone', name = '$name', vs_users_id = '$user_id', vs_lang_code = '$lang'  where id = $id limit 1 ;";
			mysql_query($sql);
		}
	...
	function isValidSurveyID($surveyID){
		$sql = "SELECT sl.id as survey_link_id,l.video_id as video_id from survey_links sl inner join links l on l.id = sl.link_id where unique_link = '$surveyID' order by survey_link_id desc limit 1";
		$result = mysql_query($sql);
		$data = mysql_fetch_assoc($result);
		if(mysql_num_rows($result) > 0 ){
			return $data;
	...
	function addSurveySubmit($data){
		...
		$vs_data['vs_form_sets'][$data['vs_form_set_id_'.$i]] = array(
											'vs_form_set_id'=>$data['vs_form_set_id_'.$i],
											'vs_fu_surveyactions_id'=>$data['vs_fu_surveyactions_id_'.$data['answer_'.$vs_form_set_id]],
											'answer'=>$data['answer_'.$vs_form_set_id]
									  );
									  
		 $actionid = $data['vs_fu_surveyactions_id_'.$data['answer_'.$vs_form_set_id]];
		 
		
		
		}
		
		//print_r($vs_data['vs_form_sets']);
		
		$ansdata = serialize($vs_data);
		
		
		 $userID = isset($_SESSION['users']['vs_users_id']) ? $_SESSION['users']['vs_users_id'] : '0';
		 $sql = "INSERT into vs_prospect_ans(vs_prospect_id,vs_form_id,ans,form,notes,vs_user_id,link_id,vs_lang_code,vs_survey_links_id,session_id)values(".$data['vs_prospect_id'].",".$data['vs_form_id'].",'$ansdata','','$comments',".$userID.",".(isset($links_link_id) ? $links_link_id : null).",'".(isset($_SESSION['lang']) ? $_SESSION['lang'] : null)."',".$_SESSION['vs_survey_links_id'].",'".session_id()."')";
	...
	function getFormData($formID){
		...
		$sql1 = "SELECT fs.`vs_form_id`, fu_a.label AS form_title, fu_a.`vs_fu_actions_id` FROM `vs_form_set` fs INNER JOIN `vs_fu_actions` fu_a ON fs.`vs_fu_actions_id` = fu_a.`vs_fu_actions_id` WHERE `vs_form_id` = '$formID' and `role` = '' and `type` = 'elm' and `displaytype` = 'label' ";
	...
	function selectTableData($selectData, $tableName, $where){
		$sql = "SELECT $selectData FROM $tableName WHERE $where";
		$result = mysql_query($sql);
		if(mysql_num_rows($result) > 0 ){
			$formTableData = mysql_fetch_assoc($result);
	...
	function getFormID($un_survey_link){
		$surveyData = array();
		$sql = "SELECT l.vs_form_id from links l inner join survey_links sl on sl.link_id =l.id where sl.unique_link = '$un_survey_link' order by l.id asc limit 1";
		$result = mysql_query($sql);
		$row = mysql_fetch_assoc($result);
		...
		return $row['vs_form_id'];
	}

	function getProspectID($un_survey_link){
		$surveyData = array();
		$sql = "SELECT sl.vs_prospect_id from survey_links sl where sl.unique_link = '$un_survey_link'";
		$result = mysql_query($sql);
		$row = mysql_fetch_assoc($result);
		return $row['vs_prospect_id'];
	}

	function getFormDetails($formID){
		$surveyData = array();
		$sql = "SELECT * from vs_form f where f.vs_form_id = '$formID'";
		$result = mysql_query($sql);
		$row = mysql_fetch_assoc($result);
		return $row;
	}
	
	function getSurveyLinkDetails($u_link){
		$surveyData = array();
		$sql = "SELECT * from survey_links sl where sl.unique_link = '$u_link'";
		$result = mysql_query($sql);
		$row = mysql_fetch_assoc($result);
		return $row;
	}
	...
	function getFormData1($formID){
		...
		$sql_query = "(
SELECT '000010' ord, vs_form_id vs_form_id,
fs.vs_form_id parent_id, 10 id, 'form' as parent_role, 'title' as role, 'elm' as type,
'label' as displaytype, fs.title_label_id label_id,
IF(tr.label IS NOT NULL,tr.label,fu_a.label) AS label, fu_a.label AS label_default, tr.label AS label_trans
FROM vs_form fs
LEFT JOIN vs_fu_actions fu_a ON fs.title_label_id = fu_a.vs_fu_actions_id
LEFT JOIN vs_lang_trans tr ON (tr.vs_fu_actions_id = fu_a.vs_fu_actions_id AND tr.vs_lang_code = '".$_SESSION['vs_lang_code']."')
WHERE fs.vs_form_id ='$formID' LIMIT 1
)
UNION
(
SELECT '900099' ord, vs_form_id vs_form_id,
fs.vs_form_id parent_id, 99 id, 'form' as parent_role, 'submit' as role, 'elm' as type,
'label' as displaytype,fs.submit_label_id,
IF(tr.label IS NOT NULL,tr.label,fu_a.label) AS label, fu_a.label AS label_default, tr.label AS label_trans
FROM vs_form fs
LEFT JOIN vs_fu_actions fu_a ON fs.submit_label_id = fu_a.vs_fu_actions_id
LEFT JOIN vs_lang_trans tr ON (tr.vs_fu_actions_id = fu_a.vs_fu_actions_id AND tr.vs_lang_code = '".$_SESSION['vs_lang_code']."')
WHERE fs.vs_form_id ='$formID' LIMIT 1
)
UNION
(
SELECT '900090' ord, vs_form_id vs_form_id,
fs.vs_form_id parent_id, 90 id, 'form' as parent_role, 'comment' as role, 'elm' as type,
'label' as displaytype,fs.comment_label_id,
IF(tr.label IS NOT NULL,tr.label,fu_a.label) AS label, fu_a.label AS label_default, tr.label AS label_trans
FROM vs_form fs
LEFT JOIN vs_fu_actions fu_a ON fs.comment_label_id = fu_a.vs_fu_actions_id
LEFT JOIN vs_lang_trans tr ON (tr.vs_fu_actions_id = fu_a.vs_fu_actions_id AND tr.vs_lang_code = '".$_SESSION['vs_lang_code']."')
WHERE fs.vs_form_id ='$formID' LIMIT 1
)
UNION
(
SELECT '000020' ord, vs_form_id vs_form_id,
fs.vs_form_id parent_id, 20 id, 'form' as parent_role, 'instructions' as role, 'elm' as type,
'label' as displaytype, fs.instructions_label_id,
IF(tr.label IS NOT NULL,tr.label,fu_a.label) AS label, fu_a.label AS label_default, tr.label AS label_trans
FROM vs_form fs
LEFT JOIN vs_fu_actions fu_a ON fs.instructions_label_id = fu_a.vs_fu_actions_id
LEFT JOIN vs_lang_trans tr ON (tr.vs_fu_actions_id = fu_a.vs_fu_actions_id AND tr.vs_lang_code = '".$_SESSION['vs_lang_code']."')
WHERE fs.vs_form_id ='$formID' LIMIT 1
)";
		...
		function getFormDataElement($formID){
		...
		$sql_query = "(
SELECT CONCAT_WS('','00',lpad(fs.ord,2,'0'),'00') ord,fs.vs_form_id vs_form_id,
fs.vs_form_id parent_id, fs.vs_form_set_id id, 'form' as parent_role, fs.role, fs.type as type,
fs.displaytype as displaytype, fu_a.vs_fu_actions_id,
IF(tr.label IS NOT NULL,tr.label,fu_a.label) AS label, fu_a.label AS label_default, tr.label AS label_trans
FROM vs_form_set fs
LEFT JOIN vs_fu_actions fu_a ON fs.vs_fu_actions_id = fu_a.vs_fu_actions_id
LEFT JOIN vs_lang_trans tr ON (tr.vs_fu_actions_id = fu_a.vs_fu_actions_id AND tr.vs_lang_code = '".$_SESSION['vs_lang_code']."')
WHERE fs.vs_form_id = '$formID'
)

UNION

(
SELECT CONCAT_WS('','00',lpad(fs.ord,2,'0'),lpad(fsa.ord,2,'0')) ord, fs.vs_form_id vs_form_id,
fs.vs_form_set_id parent_id, fsa.vs_fu_surveyactions_id id, 'subset' as parent_role, 'option' as role, 'elm' type,
'label' as displaytype, fu_a.vs_fu_actions_id,
IF(tr.label IS NOT NULL,tr.label,fu_a.label) AS label, fu_a.label AS label_default, tr.label AS label_trans
FROM vs_form_set fs
join vs_fu_sets fm USING (vs_fu_sets_id)
join vs_fu_surveyactions fsa using (vs_fu_sets_id)
join vs_fu_actions fu_a on (fsa.vs_fu_actions_id=fu_a.vs_fu_actions_id)
left join vs_lang_trans tr ON (tr.vs_fu_actions_id=fu_a.vs_fu_actions_id and tr.vs_lang_code='".$_SESSION['vs_lang_code']."')
where fs.vs_form_id = '$formID' and fs.type='set'
)
ORDER BY ord";
